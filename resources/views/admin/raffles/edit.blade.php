...
@section('content-header')
   Editando o sorteio {{ $raffle->name }} do cliente {{ $client->name }}
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-xs-12">
            ...
            <!-- /.box-header -->
                <!-- form start -->
                <form method="post" action="{{ route('raffles.update', [$raffle->id, $client->id]) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="box-body">
                        <div class="form-group">
                            <label for="name">Título do sorteio</label>
                            <input type="text" class="form-control" id="name" name="name" value="{{ $raffle->name }}" placeholder="Título da galeria">
                        </div>
                        <div class="form-group">
                            <label for="name">Descrição</label>
                            <textarea class="textarea" name="description" placeholder="Descrição da galeria"  style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;">{{ $raffle->description }}</textarea>
                        </div>
                        ...
                        <div class="form-group ">
                            ...
                            <div class="input-group">
                                ...
                                <div class="input-group date" id="date_start" data-target-input="nearest" style="width:100%">
                                    <input type="text" name="end" value="{{ $raffle->end->format('d/m/Y') }}" class="form-control datetimepicker-input input-group-append" data-target="#date_start" data-toggle="datetimepicker"/>
                                </div>
                            </div>
                        </div>
